...
public class Main extends Application {
    ...
    private ComputeTask<?> task,task2;
    ...
    public void start(Stage primaryStage) {
        ...
        mandelButton.setOnAction(e -> {
            if(task2 != null)
                task2.cancel();
            task2 = new MandelbrotTask();
            final CustomLabel currentValueLabel = new CustomLabel();
            root.setCenter(currentValueLabel);
            primaryStage.sizeToScene();
            task2.getPartialResultProperty().addListener
                ((obs, os, ns) -> currentValueLabel.setText(ns.toString()));
            task2.messageProperty().addListener((obs, ov, nv) -> statusLabel.setText(nv));
            task2.progressProperty().addListener
                ((obs, ov, nv) -> progressLabel.setText(((int)(nv.doubleValue() * 100)) + "% complete"));
